...
const newPostgresClient = async (connection: Connection) => {
  ...
  if (connection.ssl) {
    ...
    try {
      ...
      // Connecting postgres via non-ssl requires `clientConfig.ssl` is undefined.
...
const getTableSchema = async (connection: Connection, databaseName: string): Promise<Schema[]> => {
  ...
  const { rows } = await client.query(
    `SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema NOT IN (${systemSchemas}) AND (table_type='BASE TABLE' or table_type='VIEW') AND table_catalog=$1;`,
    [databaseName]
  );
  ...
  for (const schema of schemaList) {
    for (const table of schema.tables) {
      ...
      const columnList = [];
      // TODO: transform it to standard schema string.
